...
def responsive_caregiver_dashboard():
...
class _CaregiverDashboardScreenState extends State<CaregiverDashboardScreen> {
  ...
  void _toggleClockIn() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: _isClockedIn ? AppTheme.successGreen : AppTheme.warningOrange,
      ),
    );
  }
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Scaffold(
          ...
          // Only show AppBar on Mobile, Desktop has side nav
          appBar: isDesktop 
              ? null 
              : AppBar(
            ...
            children: [
              ...
              if (isDesktop)
                NavigationRail(
                  ...
                  extended: constraints.maxWidth >= 1000, // Text labels on wide screens
                  backgroundColor: Colors.white,
                  ...
                  labelType: constraints.maxWidth < 1000 ? NavigationRailLabelType.all : null,
                  leading: Padding(
  ...
  Widget _buildBody() {
    // If embedded view (like CareNotes) has its own scaffold/appbar, we might want to wrap it 
    // to prevent double headers on mobile, but for now we render as is.
  ...
  Widget _buildStatsRow() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // Switch to column on very small screens, row on others
        return Wrap(
          spacing: 16,
          runSpacing: 16,
          children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () {}, style: ElevatedButton.styleFrom(backgroundColor: Colors.white, foregroundColor: AppTheme.primaryPurple, elevation: 0), child: const Text("View Details & Clock In"))),
        ],
      ),
    );
  }
...
"""
    ...
    print("Refactored Caregiver Dashboard with Responsive Navigation (Rail/BottomNav).")

if __name__ == "__main__":
    responsive_caregiver_dashboard()